...
float3 getColor(float3 p, float3 n, float3 d, float dist)
{
	...
	float s = 0.0f;
	float N;
	FBM_THIS_C(s, 8, 2.05f, (noise3d(p * 0.02f * SCALE) * 0.5f + 0.5f) / SCALE);
	...
	s = 0.0f;
	FBM_THIS_C(s, detail, 2.03f, (abs(noise3d(float3(p.y * 0.5f, p.x * 0.01f, p.z * 0.1f) * SCALE)) / SCALE);
	color1 -= s * max((200.0f-dist)/200.0f, 0.0f);
